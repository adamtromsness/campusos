...
**Status:** Cycle 8 **COMPLETE + APPROVED + FOLLOW-UP APPROVED**. Round 1 of REVIEW-CYCLE8-CHATGPT (against `cycle8-complete` at `fa0797c`) returned "Approved with major follow-ups" — no BLOCKING items, 5 major follow-ups split into 3 clean bug fixes (landed in `c424288`) + 2 architectural decisions carried as Phase 2 backlog. **Round 2 (follow-up review against `c424288` / `cycle8-approved`) returned "Approved" — final gate decision.** The reviewer confirmed all three code-level follow-ups (F2 resolver self-notification, F3 ProblemService.patch validation, F4 locationId soft-ref validation) are properly addressed in `apps/api/src/tickets/ticket.service.ts`, `apps/api/src/tickets/problem.service.ts`, and `apps/api/src/notifications/consumers/ticket-notification.consumer.ts`; F1 (IT-001 permission model) and F5 (per-parent category UNIQUE) appropriately remain as Phase 2 backlog items. All 10 steps done. Vertical-slice CAT at `docs/cycle8-cat-script.md` verified live on `tenant_demo` 2026-05-04. Cycle 8 is the **final cycle of Wave 1** — Wave 1 is now closed. The platform ships the complete core operational stack (SIS + Classroom + Communications + HR + Scheduling + Enrollment + Payments + Profile/Household + Tasks + Approvals + Service Tickets) and is ready for Wave 2 (Student Services — Behaviour, Health, Counselling, Library, Athletics & Clubs). Tag `cycle8-complete` at `fa0797c`; `cycle8-approved` at `c424288` (REVIEW-CYCLE8 closeout commit, both rounds approved). **REVIEW-CYCLE8 fixes (2026-05-04):** (Follow-up 2 — bug) `TicketNotificationConsumer.fanOutResolved` now compares `ctx.requesterId` (a `platform_users.id`) against a new `resolvedByAccountId: actor.accountId` field on the `tkt.ticket.resolved` payload (added at both `TicketService.resolve` and `ProblemService.resolveBatch`). The prior comparison against `assigneeId` (an `hr_employees.id`) was a category mismatch and never matched, so the self-notification suppression path was dead — admins resolving their own tickets received a redundant "your ticket has been resolved" notification. Verified live: Sarah submits + resolves her own ticket → wire envelope shows `resolvedByAccountId === requesterId`; queue contains 0 `ticket.resolved` rows for her id. (Follow-up 3 — bug) `ProblemService.patch` now opens `executeInTenantTransaction`, locks the row + reads existing `root_cause` / `assigned_to_id` / `vendor_id` atomically, computes the post-patch effective shape, and rejects KNOWN_ERROR without effective rootCause + the assignee/vendor mutex + bogus FK targets — all with friendly 400s before the UPDATE fires. The DB CHECK is the schema-side belt-and-braces. Verified live with 5 scenarios on the seeded "Network switch failure" problem: bare KNOWN_ERROR rejected with `"KNOWN_ERROR requires a root_cause"`; KNOWN_ERROR + rootCause in same call accepted; bogus assignedToId rejected with `"assignedToId does not match an hr_employees row"`; bogus vendorId on a problem with existing assignedToId rejected with the mutex error first (correct ordering — the more useful message wins). (Follow-up 4 — bug) `TicketService.create` adds an existence check on `locationId` against `sch_rooms` inside the existing tenant context. Bogus `00000000-…` UUID returns `400 "locationId does not match a room in this school"`; real Room 101 id accepted cleanly. **Architectural follow-ups carried to Phase 2:** (Follow-up 1) Permission model rename or split — `IT-001` is functioning as the de-facto helpdesk function across IT/Facilities/HR-support tickets but the name is IT-only. Three resolution options on the table; decision deferred until the M65 Facilities cycle in Wave 2 surfaces actual cross-functional admin overlap. (Follow-up 5) Category uniqueness — `tkt_categories` UNIQUE(school_id, name) is global across the tree. The seed's 3 + 11 names don't collide; if a real-world catalogue does, switch to a partial UNIQUE on `(school_id, parent_category_id, name)` for non-null parent + a partial UNIQUE on `(school_id, name) WHERE parent_category_id IS NULL` for top-level. **Four accepted deviations move to Wave 2 Phase 2 punch list:** consumer tenant routing trusts the `tenant-subdomain` header without validating against `platform_tenant_routing`; `KafkaProducerService.emit()` is best-effort and never throws — finance / emergency / workflow-critical paths will need an outbox pattern; `TenantResolverMiddleware` accepts the `X-Tenant-Subdomain` header in production; `PermissionGuard` denies platform-scope routes that lack a tenant context — needs a platform-scope auth path before v11 CRM / OPS modules ship. See `REVIEW-CYCLE8-CHATGPT.md` for the full triage table + verification trail. Cycles 0–7 are COMPLETE + APPROVED. Cycle 8 ships the M60 Service Tickets module — a shared reactive ticketing engine that IT, facilities, and HR support all use. NOT a facilities management system (that is M65, a future cycle). This module handles reactive requests like "my projector is broken," "Room 204 needs a new light bulb," "I can't log in." It integrates with Cycle 7's Task Worker (ticket assignment creates a task on the assignee's to-do list) and the Cycle 7 Approval Workflows engine (ticket escalation can route through an approval chain).
