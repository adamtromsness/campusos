...
(BLOCKING 1 — soft-ref validation on requisitions) New `assertBudgetLineInCurrentTenant(budgetLineId)` helper added to `apps/api/src/finance/validation.ts`; verifies the line exists in this tenant + its parent budget is APPROVED (not DRAFT or REJECTED). `RequisitionService.create` now validates `budgetLineId` + every line's `preferredVendorId` (via the existing `FinanceValidationService.assertActiveSupplier`) BEFORE the INSERT, throwing friendly 400s with the specific field name. `RequisitionService.addLine` validates `preferredVendorId` similarly. Verified live: bogus `budgetLineId` returns `400 "budgetLineId does not match a budget line in this school"`; bogus `lines[].preferredVendorId` returns `400 "lines[].preferredVendorId does not match a supplier in this school"`.

(BLOCKING 2 — PO source-line + GL validation) `PurchaseOrderService.create` now: (a) validates each line's `glAccountId` (when supplied) is an active EXPENSE/ASSET/LIABILITY account in this tenant via `FinanceValidationService.assertActiveAccount` BEFORE the tx; (b) when `requisitionId` is supplied, validates inside the tx that the requisition belongs to this school AND has status in `('ADMIN_APPROVED', 'DISTRICT_APPROVED')` — DRAFT / SUBMITTED / DEPT_APPROVED requisitions cannot be ordered; (c) when `requisitionLineId` is set on any PO line, validates every supplied id belongs to the parent requisition via a single `id = ANY(...) AND requisition_id = ...` query that surfaces orphan ids in the 400 message; (d) refuses standalone POs (no `requisitionId`) where any line carries `requisitionLineId`. Verified live across 5 paths: PO from DRAFT requisition `400 "Cannot create a PO from a requisition in status=DRAFT"`; bogus `glAccountId` 400; REVENUE-typed `glAccountId` `400 "(4000 Tuition Revenue) has account_type=REVENUE; expected one of EXPENSE, ASSET, LIABILITY"`; orphan `requisitionLineId` 400 with the offending UUID inlined; standalone PO + `requisitionLineId` 400 redirect.

(BLOCKING 3 — vendor performance scoring) `GoodsReceiptService.create` rewritten so vendor-performance counters land at the correct grain: `total_orders` bumps **once per PO** (only when this is the first receipt for the PO — a `count(*) FROM prc_goods_receipts WHERE purchase_order_id = $1` check before the receipt INSERT determines it); `on_time_deliveries` and `late_deliveries` bump **only when the receipt completes the PO** (newStatus === 'RECEIVED'), using the `today <= expected_delivery_date` check on the final delivery; `accepted_count` and `rejected_count` continue to bump per receipt (line-level — the natural granularity for quality scoring). Average scores recompute against the new totals. The existing UPSERT pattern preserved. Verified live with a 3-receipt partial-delivery scenario on a 10-unit PO: pre-state `(total_orders=1, on_time=1)`; after Receipt 1 (4 units, partial) → `(total_orders=2, on_time=1)` (orderBump=1, on_time unchanged); after Receipt 2 (3 more, still partial) → `(total_orders=2, on_time=1)` (no order bump, no on_time bump); after Receipt 3 (final 3, PO becomes RECEIVED) → `(total_orders=2, on_time=2, accepted=29, rejected=1, avg_qual=0.9667, avg_del=1.0)`. PO `total_orders` is now correctly 2 (1 seed + 1 new) instead of the previous-buggy 4.

(BLOCKING 4 — return concurrency) `ReturnService.create` now opens `executeInTenantTransaction`, takes `SELECT … FROM prc_goods_receipt_lines rl … FOR UPDATE OF rl LIMIT 1` to lock the parent receipt line, and re-reads `already_returned` AFTER the lock. Concurrent return-create calls on the same receipt line serialise on the row lock and the loser sees the winner's row before the cap recheck. Verified live: 5 parallel POSTs requesting 1 unit each against a receipt line with `quantity_received=4` landed exactly 4× 201 + 1× 400 ("quantity_returned 1 exceeds remaining 0"); a 6th sequential attempt also rejected with the same message. Total returned units stayed at the 4-unit cap.

(BLOCKING 5 — settings validation) `ProcurementSettingsService.update` now applies service-layer policy validation BEFORE the UPDATE: `poNumberPrefix` must be non-blank after trim, ≤20 characters, and match `/^[A-Za-z0-9-]+$/` (ASCII letters, digits, dashes only — protects against semicolons and other characters that would break PO-number formatting); when `autoPoThreshold` and `requireThreeQuotesAbove` are both set on the row after the patch (resolved against the current row for partial PATCHes), `requireThreeQuotesAbove >= autoPoThreshold` is enforced (the quote-required threshold cannot sit below the auto-PO threshold). Class-validator's `@Min(0)` on the DTO already covered non-negative numerics. Verified live across 5 paths: blank prefix `400 "must not be blank"`; semicolon-laden prefix `400 "must contain only ASCII letters, digits, and dashes"`; >20-char prefix `400 "shorter than or equal to 20 characters"` (DTO-level @MaxLength); cross-field violation `400 "requireThreeQuotesAbove (500) must be greater than or equal to autoPoThreshold (1000)"`; valid update `200`.

(MAJOR 8 — tighten ORDERED cascade) `PurchaseOrderService.transition` ISSUE branch tightened — the linked-requisition cascade now flips ONLY requisitions in `('ADMIN_APPROVED', 'DISTRICT_APPROVED')` to ORDERED. DRAFT / SUBMITTED / DEPT_APPROVED requisitions stay in their current state when their PO issues (the requisition-state guard from BLOCKING 2 prevents the create path from landing here in the first place; this is defence-in-depth for any DB-direct insert path). Verified live by directly INSERTing a DRAFT requisition + DRAFT PO via SQL (bypassing the new BLOCKING-2 guard) and ISSUE-ing the PO via the API — PO flips to ISSUED but linked requisition stays DRAFT.

**3 MAJORs acknowledged + carried to Phase 2 punch list:** MAJOR 6 (workflow engine integration on requisitions — Cycle 7 wsk_approval_requests engine connection deferred; current direct-admin transitions are the documented stand-in), MAJOR 7 (`prc.distribution.completed` outbox path — best-effort emit today; once destination workers go live this should move to outbox-backed semantics), MAJOR 9 (`GREATEST(encumbered - amt, 0)` clamp on commitment release — the schema's non-negative CHECK on `encumbered_amount` is the safety net; pre-pilot replace with a checked update that fires a finance exception on accounting drift instead of silently clamping).

See `REVIEW-CYCLE27-CHATGPT.md` for the triage table + verification trail. Tagging `cycle27-approved` after Round 2 verdict.

## Cycle 27 — original Round 1 build state. Wave 6 (Finance & Commerce) continuation cycle. Cycle 27 builds the M86 Procurement module — all 12 ERD tables in scope per the basic procurement scope (ADR-061). The Procurement Officer / Purchasing Clerk is the **eleventh specialist operator persona** after the nurse, counsellor, librarian, athletic director, enrolment officer, transportation coordinator, food service manager, facilities manager, IT administrator, and CFO/Business Manager. **The cross-module supply chain hub** — when goods are received and distributed, `prc.distribution.completed` fires with a `destination_module` payload that 8 destination modules (tech / trn / fds / lib / ath / ext / fac / str) can consume to create their own inventory rows. Procurement never writes directly to another module's tables.
