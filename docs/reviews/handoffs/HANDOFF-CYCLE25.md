...
**Status:** Cycle 25 **COMPLETE + APPROVED at `bfd393f`** — Wave 5 (Academic Advanced) closeout cycle. Round 1 of REVIEW-CYCLE25-CHATGPT (against `cycle25-complete` at `5aee99c`) returned **Reject pending fixes** with 5 BLOCKING + 4 MAJOR. Round 2 against `bfd393f` returned **Approved** — reviewer cache-busted each affected file in code on Round 2 and confirmed every fix matches. All 5 BLOCKING + the 3 actionable MAJORs (6 / 7 / 8) landed in the fix commit with live verification on `tenant_demo` 2026-05-06: actor-aware reads inheriting the publication visibility lattice (parents/students see PUBLISHED only; non-writer non-collaborator readers get collapsed 404 on non-PUBLISHED + filtered to is_approved=true sections); student section creation gated by CONTRIBUTOR/EDITOR collaborator role; section approval restricted to admin / publication creator / EDITOR / REVIEWER (generic `pub-002:write` no longer enough); collaborator + contributor IDs tenant-validated via shared `assertAccountInCurrentTenant`; GROUP_MEMBERSHIP audience resolution uses `grp_members.person_id` directly as account_id (Cycle 18 semantics — the iam_person join had been returning zero rows); distribution reads gated to admin / pub-003:write / EDITOR collaborator; rule_value validated for ROLE/GRADE/CLASS/GROUP_MEMBERSHIP; subscription series existence check. MAJOR 9 (audience-resolved-outside-tx eventual consistency) acknowledged + carried to Phase 2 punch list, alongside the PUB-003 catalogue rename (currently labelled "Parent Portal" while runtime treats it as Distribution). See `REVIEW-CYCLE25-CHATGPT.md` for the triage table + verification trail. Tagged `cycle25-complete` at `5aee99c` and `cycle25-approved` at `bfd393f`. All 10 steps shipped + vertical-slice CAT verified live on `tenant_demo` 2026-05-06. Cycle 25 ships the M42 Publications module — 11 of the 15 ERD tables in scope (4 deferred to Cycle 25.1: pub_publication_versions, pub_media_assets, pub_approval_delegations, pub_templates). Recurring publication series (the brand) carrying numbered editions (specific issues), individual publications (content documents) with multi-section content structure and per-section ownership + contributor attribution, role-based approval workflow per ADR-035 (staff publish directly, student-authored content requires approval), targeted distribution lists with rule-based audience resolution (ROLE / GRADE / CLASS / GROUP_MEMBERSHIP), per-recipient delivery tracking (PENDING / DELIVERED / OPENED / BOUNCED), and series subscriptions with self-service opt-out. **Closes Wave 5 (Academic Advanced).**
