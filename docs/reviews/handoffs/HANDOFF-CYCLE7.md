...
| Verb | Path                                   | Permission      | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---- | -------------------------------------- | --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| POST | `/approvals`                           | `ops-001:admin` | Submit a new approval request — **admin only** (REVIEW-CYCLE7 BLOCKING 2 tightened from `ops-001:write`). Engine selects the active workflow template by `request_type`, validates `referenceTable` against the `ALLOWED_REFERENCE_TABLES` allowlist + strict snake_case regex + per-tenant row existence, creates the request row + Step 1 with a resolved approver in one tx. Emits `approval.step.awaiting`. Domain modules submit programmatically via `WorkflowEngineService.submit()` and bypass the gate. |
...
| POST | `/approvals/:id/withdraw`              | `ops-001:write` | Requester pulls back a still-PENDING request. Marks every AWAITING step SKIPPED + status=WITHDRAWN. **Emits `approval.request.resolved` with `status='WITHDRAWN'`** (Step 10 close-out wired the WITHDRAWN cascade) so source-module consumers (e.g. `LeaveApprovalConsumer`) cancel the underlying domain row.                                                                                                                                                                                                  |
...
6. **L7** Balance moved from `pending=0.00 used=2.00` to `pending=0.00 used=3.00` (the seeded pending=1 from L2 became used). `reviewed_at` populated, `reviewed_by` set to the actual final approver's account id (REVIEW-CYCLE7 MAJOR 3 — engine emits `approverAccountId` on `approval.request.resolved`; the original Step 7 ship used the requester id as a placeholder).
...
- ~~Final-approver id in the resolved payload.~~ **CLOSED in REVIEW-CYCLE7 MAJOR 3.** `WorkflowEngineService.advanceStep` and `withdraw` now emit `approverAccountId: actor.accountId` on `approval.request.resolved`; `LeaveApprovalConsumer` uses it (with `?? p.requesterId` as transition fallback) so `hr_leave_requests.reviewed_by` records the actual final approver, not the requester.
...
### Post-cycle review fixes (Round 1 — REJECT, fixes applied this commit)

External review at `c6e7732` returned REJECT pending **2 BLOCKING + 3 MAJOR**. All 5 valid; all 5 fixed and live-verified on `tenant_demo` 2026-05-03. Triage and verification trail in `REVIEW-CYCLE7-CHATGPT.md`.

| #          | Severity | What changed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| BLOCKING 1 | CRITICAL | `TaskWorker.createAcknowledgements` and `createTasks` now rethrow on INSERT failure and release the per-(owner, source_ref_id) Redis dedup claim before rethrowing, so `processWithIdempotency` does NOT claim the Kafka event_id and redelivery retries the failed owner. Owners whose insert already succeeded keep their Redis claim and skip cleanly on retry. Acknowledgement inserts gain matching Redis SET NX dedup at `tsk:auto-ack:{subdomain}:{ownerId}:{sourceRefId}` plus a new `lookupExistingAck` helper so a redelivered event whose ack already landed can link the existing ack id into the companion CREATE_TASK action. |
| BLOCKING 2 | HIGH     | `POST /approvals` re-gated from `ops-001:write` (broadly held) to `ops-001:admin` (admin-only). Domain modules submit programmatically via `WorkflowEngineService.submit()` and bypass. Defence-in-depth: new `ALLOWED_REFERENCE_TABLES` allowlist (`hr_leave_requests`, `sis_absence_requests`, `sis_child_link_requests`) + strict `^[a-z][a-z0-9_]{0,63}$` regex check + per-tenant existence verification on the referenced row inside the open transaction. Verified live: teacher / parent POST 403; admin with disallowed table → 400 allowlist; admin with bogus row id → 400 not found.                                            |
| MAJOR 3    | MEDIUM   | `WorkflowEngineService.advanceStep` and `withdraw` now include `approverAccountId: actor.accountId` on the `approval.request.resolved` envelope payload. Verified live: leave row's `reviewed_by` = the actual approver's accountId (not the requester). The consumer's existing `?? p.requesterId` fallback stays as transition defence for in-flight events from before the fix.                                                                                                                                                                                                                                                          |
| MAJOR 4    | MEDIUM   | `TaskService.create()` admin delegation calls new `assertAssigneeInCurrentTenant(accountId)` mirroring the Cycle 6.1 `ProfileService.assertTargetInCurrentTenant` pattern — verifies the supplied `platform_users.id` has a domain projection (`sis_students` / `sis_guardians` / `hr_employees`) in the calling tenant. Bogus UUIDs → `400 "assigneeAccountId does not belong to a user in this school"`.                                                                                                                                                                                                                                  |
| MAJOR 5    | DOC-ONLY | `POST /approvals/:id/withdraw` `@ApiOperation` summary rewritten — engine DOES emit `approval.request.resolved` with `status='WITHDRAWN'` (per Step 10 close-out). Previous summary was stale doc text from before Step 10.                                                                                                                                                                                                                                                                                                                                                                                                                 |

After these fixes, `cycle7-complete` tag was force-moved to the closeout commit. Round 2 verdict pending.
