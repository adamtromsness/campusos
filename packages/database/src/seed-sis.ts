...
var NEW_GUARDIANS: GuardianSpec[] = [
  {
    firstName: 'Carmen',
    lastName: 'Rodriguez',
    email: 'crodriguez@parents.demo.campusos.dev',
    relationship: 'PARENT',
  },
  {
    firstName: 'Tasha',
    lastName: 'Johnson',
    email: 'tjohnson@parents.demo.campusos.dev',
    relationship: 'PARENT',
  },
  {
    firstName: 'Patrick',
    lastName: "O'Connor",
    email: 'poconnor@parents.demo.campusos.dev',
    relationship: 'PARENT',
  },
  {
    firstName: 'Raj',
    lastName: 'Patel',
    email: 'rpatel@parents.demo.campusos.dev',
    relationship: 'PARENT',
  },
  {
    firstName: 'Marcus',
    lastName: 'Williams',
    email: 'mwilliams@parents.demo.campusos.dev',
    relationship: 'PARENT',
  },
  {
    firstName: 'Janelle',
    lastName: 'Williams',
    email: 'jwilliams@parents.demo.campusos.dev',
    relationship: 'PARENT',
  },
  {
    firstName: 'Rivka',
    lastName: 'Goldberg',
    email: 'rgoldberg@parents.demo.campusos.dev',
    relationship: 'PARENT',
  },
  {
    firstName: 'Yuki',
    lastName: 'Tanaka',
    email: 'ytanaka@parents.demo.campusos.dev',
    relationship: 'PARENT',
  },
  {
    firstName: 'Hiro',
    lastName: 'Tanaka',
    email: 'htanaka@parents.demo.campusos.dev',
    relationship: 'PARENT',
  },
];
...
var FAMILIES: FamilySpec[] = [
  { name: 'Chen Family', studentNames: ['Maya Chen'], guardianNames: ['David Chen'] },
  {
    ...
    studentNames: ['Ethan Rodriguez', 'Isabella Rodriguez'],
    guardianNames: ['Carmen Rodriguez'],
  },
  {
    name: 'Johnson Family',
    studentNames: ['Aaliyah Johnson', 'Aiden Johnson'],
    guardianNames: ['Tasha Johnson'],
  },
  { name: "O'Connor Family", studentNames: ["Liam O'Connor"], guardianNames: ["Patrick O'Connor"] },
  {
    name: 'Patel Family',
    studentNames: ['Sofia Patel', 'Zara Patel'],
    guardianNames: ['Raj Patel'],
  },
  {
    name: 'Williams Family',
    studentNames: ['Noah Williams', 'Olivia Williams'],
    guardianNames: ['Marcus Williams', 'Janelle Williams'],
  },
  {
    name: 'Goldberg Family',
    studentNames: ['Emma Goldberg', 'Mason Goldberg'],
    guardianNames: ['Rivka Goldberg'],
  },
  {
    name: 'Tanaka Family',
    studentNames: ['Lucas Tanaka', 'Jackson Tanaka', 'Chloe Tanaka'],
    guardianNames: ['Yuki Tanaka', 'Hiro Tanaka'],
  },
];
...
async function seedSis() {
  ...
  if (existingCount[0] && existingCount[0].count > 0n) {
    console.log(
      '  SIS data already seeded (' + existingCount[0].count + ' sis_students rows) — skipping',
    );
    return;
  }
  ...
  var mayaPlatformStudent = await client.platformStudent.findFirst({
    where: { personId: mayaPerson.id },
  });
  ...
  console.log(
    '  ' +
      NEW_GUARDIANS.length +
      ' guardian identities resolved (' +
      newGuardianCreated +
      ' newly created)',
  );
  ...
  await client.$executeRawUnsafe(
    'INSERT INTO ' +
      TENANT_SCHEMA +
      '.sis_terms (id, academic_year_id, name, start_date, end_date, term_type) VALUES ($1::uuid, $2::uuid, $3, $4::date, $5::date, $6)',
    termFallId,
    academicYearId,
    'Fall 2025',
    '2025-08-15',
  ...
  for (var di = 0; di < deptDefs.length; di++) {
    ...
    await client.$executeRawUnsafe(
      'INSERT INTO ' +
        TENANT_SCHEMA +
        '.sis_departments (id, school_id, name) VALUES ($1::uuid, $2::uuid, $3)',
      deptId,
      schoolId,
      d.name,
    );
  ...
  for (var ci = 0; ci < COURSES.length; ci++) {
    ...
    await client.$executeRawUnsafe(
      'INSERT INTO ' +
        TENANT_SCHEMA +
        '.sis_courses (id, school_id, department_id, code, name, credit_hours, grade_level) VALUES ($1::uuid, $2::uuid, $3::uuid, $4, $5, $6::numeric, $7)',
      courseId,
      schoolId,
      deptIdByCode[c.dept]!,
      c.code,
  ...
  for (var ki = 0; ki < CLASSES.length; ki++) {
    ...
    await client.$executeRawUnsafe(
      'INSERT INTO ' +
        TENANT_SCHEMA +
        '.sis_classes (id, school_id, course_id, academic_year_id, term_id, section_code, room, max_enrollment) VALUES ($1::uuid, $2::uuid, $3::uuid, $4::uuid, $5::uuid, $6, $7, $8)',
      classId,
      schoolId,
      courseIdByCode[cl.courseCode]!,
      academicYearId,
      termSpringId,
      cl.period,
      cl.room,
  ...
  await client.$executeRawUnsafe(
    'INSERT INTO ' +
      TENANT_SCHEMA +
      '.sis_staff (id, person_id, account_id, school_id, staff_type) VALUES ($1::uuid, $2::uuid, $3::uuid, $4::uuid, $5)',
    teacherStaffId,
    teacherPersonId,
    teacherUserId,
    schoolId,
  ...
  for (var ti = 0; ti < CLASSES.length; ti++) {
    ...
    await client.$executeRawUnsafe(
      'INSERT INTO ' +
        TENANT_SCHEMA +
        '.sis_class_teachers (id, class_id, teacher_employee_id, is_primary_teacher) VALUES ($1::uuid, $2::uuid, $3::uuid, $4)',
      ctId,
      classIdByPeriod[tcl.period]!,
      teacherPersonId,
      true,
    );
  }
  ...
  await client.$executeRawUnsafe(
    'INSERT INTO ' +
      TENANT_SCHEMA +
      '.sis_students (id, platform_student_id, school_id, student_number, grade_level, enrollment_status) VALUES ($1::uuid, $2::uuid, $3::uuid, $4, $5, $6)',
    mayaSisId,
    mayaPlatformStudent.id,
    schoolId,
    'S-1001',
    '9',
    'ENROLLED',
  ...
  await client.$executeRawUnsafe(
    'INSERT INTO ' +
      TENANT_SCHEMA +
      '.sis_guardians (id, person_id, account_id, school_id, relationship, preferred_contact_method) VALUES ($1::uuid, $2::uuid, $3::uuid, $4::uuid, $5, $6)',
    davidSisGuardianId,
    davidPerson.id,
    davidUser.id,
    schoolId,
  ...
  for (var fi = 0; fi < FAMILIES.length; fi++) {
    ...
    await client.$executeRawUnsafe(
      'INSERT INTO ' +
        TENANT_SCHEMA +
        '.sis_families (id, family_name, created_by) VALUES ($1::uuid, $2, $3::uuid)',
      familyId,
      f.name,
      createdBy,
    );
    ...
    for (var sm = 0; sm < f.studentNames.length; sm++) {
      ...
      await client.$executeRawUnsafe(
        'INSERT INTO ' +
          TENANT_SCHEMA +
          '.sis_family_members (id, family_id, person_id, person_type, relationship_to_family, is_primary_contact) VALUES ($1::uuid, $2::uuid, $3::uuid, $4, $5, $6)',
        fmId,
        familyId,
        personByName[stuName]!,
        'STUDENT',
        'CHILD',
        false,
      );
    ...
    for (var sj = 0; sj < fam.studentNames.length; sj++) {
      for (var gj = 0; gj < fam.guardianNames.length; gj++) {
        ...
        await client.$executeRawUnsafe(
          'INSERT INTO ' +
            TENANT_SCHEMA +
            '.sis_student_guardians (id, student_id, guardian_id, has_custody, is_emergency_contact, receives_reports, portal_access, portal_access_scope) VALUES ($1::uuid, $2::uuid, $3::uuid, $4, $5, $6, $7, $8)',
          sgId,
          sisStudentIdByName[fam.studentNames[sj]!]!,
          sisGuardianIdByName[fam.guardianNames[gj]!]!,
          true,
          true,
          true,
          true,
  ...
  for (var pi = 0; pi < periods.length; pi++) {
    ...
    for (var ri = 0; ri < roster.length; ri++) {
      ...
      await client.$executeRawUnsafe(
        'INSERT INTO ' +
          TENANT_SCHEMA +
          '.sis_enrollments (id, student_id, class_id, status) VALUES ($1::uuid, $2::uuid, $3::uuid, $4)',
        enrId,
        sisStudentIdByName[roster[ri]!]!,
        classId,
        'ACTIVE',
      );
      enrollmentCount++;
  ...
  for (var pa = 0; pa < periods.length; pa++) {
    ...
    for (var ai = 0; ai < pRoster.length; ai++) {
      ...
      await client.$executeRawUnsafe(
        'INSERT INTO ' +
          TENANT_SCHEMA +
          '.sis_attendance_records (id, school_id, school_year, student_id, class_id, date, period, status, confirmation_status) VALUES ($1::uuid, $2::uuid, $3::date, $4::uuid, $5::uuid, $6::date, $7, $8, $9)',
        arId,
        schoolId,
        schoolYearDate,
        sisStudentIdByName[pRoster[ai]!]!,
        pClassId,
        todayDate,
        pPeriod,
        'PRESENT',
