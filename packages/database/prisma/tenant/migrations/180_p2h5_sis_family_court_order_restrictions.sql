 ...
 * health_records=false, transport_contact=false, communications=false,
 * conference_attendance=false. Missing keys default to permissive (no
 * restriction) — only an explicit false blocks the capability for the
 * matching guardian. Schema-side default is the empty object so
 ...
 *
 * NOTE on splitter compatibility — every semicolon was removed from
 * the comment text because the tenant migration splitter is line-based
 * and would otherwise break this block into multiple invalid statements.
 */
