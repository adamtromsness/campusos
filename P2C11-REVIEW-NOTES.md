...
## Round 1 fix log (2026-05-11)

Reviewer verdict: **FAIL** on `af19bb5` with 6 BLOCKING + 2 MAJOR. Fix commit
applies every item with regression tests.

### BLOCKING fixes

| #   | Area                          | Files / SQL changes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Tests                                                                                                                                                                           |
| --- | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | RepairService school-scoping  | `apps/api/src/transport/repair.service.ts` — every read joins `trn_vehicles v ON v.id = r.vehicle_id` with `v.school_id = $tenant.schoolId`. `create()` vehicle lock + safety-critical UPDATE both predicate on `school_id`. `patch()` repair lock JOINs through `trn_vehicles`; vehicle release UPDATE on COMPLETED also school-scoped.                                                                                                                                                                             | `transport-advanced.spec.ts` — 3 new pinned regressions in the `REVIEW-P2C11 ROUND 1` describe block.                                                                           |
| 2   | DriverHoursService scoping    | `apps/api/src/transport/driver-hours.service.ts` — new `assertDriverInCurrentSchool(driverId)` joins `hr_employees` on `school_id`. SELECT base JOINs through `hr_employees`. `complete()` lock + weekly-sum + reload all carry `e.school_id = $tenant.schoolId`. `create()` validates driver before INSERT.                                                                                                                                                                                                         | `transport-advanced.spec.ts` — 3 new pinned regressions (cross-school driver refused, lock JOIN shape, STAFF without TRN-003).                                                  |
| 3   | Durable outbox emits          | `apps/api/src/transport/parts.service.ts` + `driver-hours.service.ts` + `route-generation.service.ts` + `geofence.service.ts` — every emit moves from best-effort `KafkaProducerService.emit` to `OutboxService.enqueueInTx` INSIDE the same tenant tx. 4 new deterministic event-id helpers (sha256 → v5-shaped UUID). 5 topics migrated: `trn.parts.low`, `trn.driver.hours_approaching_limit`, `trn.generation.completed`, `trn.geofence.entered`, `trn.geofence.exited`.                                         | Per-helper stability + v5-shape tests across the three spec files.                                                                                                              |
| 4   | Parent token school-defensive | Migration `139_trn_parent_tracking_school_id.sql` adds NOT NULL `school_id` column + backfills via `trn_routes.school_id` + partial INDEX. `apps/api/src/transport/parent-tracking.service.ts` — every revoke / list / view / create write or read filters on `school_id = $tenant.schoolId`. Public `viewByToken` JOINs through `trn_routes` + `trn_vehicles` for defence in depth.                                                                                                                                 | `transport-advanced-c.spec.ts` — 4 new pinned regressions (viewByToken SQL shape, revokeToken UPDATE shape, listForStudent SELECT shape, createToken INSERT carries school_id). |
| 5   | RouteGeneration cancel/scope  | `apps/api/src/transport/route-generation.service.ts` — `cancelRequest` lock + UPDATE both add `school_id`. `addManualCandidate` RUNNING-flip UPDATE adds it. `markRequestCompleted` UPDATE + reread both add it. `approveCandidate` candidate lock JOINs through `trn_generation_requests` for the school predicate; vehicle + driver validation also school-scoped. `rejectCandidate` lock JOINs through the request.                                                                                               | `transport-advanced-b.spec.ts` — 1 new pinned regression on cancel lock + UPDATE SQL shape.                                                                                     |
| 6   | TRN-\* perm checks            | RepairService / PartsService / DriverHoursService / RouteGenerationService / GeofenceService / ParentTrackingService — every `assertCanManage*` helper now calls `permissionCheckService.hasAnyPermissionInTenant(accountId, schoolId, ['trn-XXX:write', 'trn-XXX:admin'])` instead of `personType === 'STAFF'`. Fleet management → TRN-002; route gen + parent tokens → TRN-001; driver hours → TRN-003. Staff role keeps the writes through the existing IAM grants; the gate is now an explicit permission check. | 4 new pinned regressions in the spec files asserting STAFF actors without the relevant TRN write perm are refused with `ForbiddenException`.                                    |

### MAJOR fixes

| #   | Area                                    | Files                                                                                                                                                                                                                                                                             | Tests                                                                                                                                                                 |
| --- | --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 7   | FleetStatus related-reads school-scoped | `apps/api/src/transport/fleet-status.service.ts` — every read of `trn_vehicle_documents`, `trn_driver_credentials`, `trn_vehicle_repairs`, `trn_vehicle_positions`, `trn_routes`, `trn_vehicle_fuel_logs` JOINs through `trn_vehicles vv` with `vv.school_id = $tenant.schoolId`. | Covered indirectly by the existing FleetStatus tests + visual inspection — no SQL-shape regression added because the materialiser is exercised end-to-end in the CAT. |
| 8   | ETAService.upsert final reload          | `apps/api/src/transport/eta.service.ts` — JOIN `trn_vehicles vv` on the post-UPSERT SELECT.                                                                                                                                                                                       | Covered by the existing ETA upsert keystone test which now exercises the school-defensive reload.                                                                     |

### Carry-overs to Phase 2 punch list

None new — all six BLOCKING + the two MAJOR items addressed in Round 1.

### Final verdict — pending Round 2

CI parity green: `pnpm format:check` clean, `pnpm lint:logs` 733 files clean,
