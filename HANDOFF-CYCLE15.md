...
**Status:** Cycle 15 **COMPLETE + APPROVED at `39b0d90` (REVIEW-CYCLE15-CHATGPT — final verdict).** Round 1 (against `cycle15-complete` at `93b1bd5`) returned **Reject pending fixes** with 3 BLOCKING row-scope / sensitive-data issues + 5 MAJOR follow-ups. All actionable items addressed in the fix commit `39b0d90` (live-verified on `tenant_demo` 2026-05-06). Round 2 against `39b0d90` returned **Approved** — reviewer confirmed all 3 BLOCKING findings are closed in code plus the four code-level MAJOR follow-ups; MAJORs 6 + 8 correctly carried as Phase 2 punch list items 20 + 21. Tagged `cycle15-complete` at `93b1bd5` and `cycle15-approved` at `39b0d90`. All 10 steps done. Vertical-slice CAT at `docs/cycle15-cat-script.md` covers the 10 plan scenarios end-to-end against `tenant_demo`. Cycle 15 builds the M41 Meetings module — 11 of the 13 ERD tables in scope. Final cycle totals: **11 new base tables**, **11 new intra-tenant FKs**, **0 cross-schema FKs**, **~32 new endpoints**, **1 new Kafka emit topic** (`mtg.meeting.scheduled`), **4 new web routes**, **17 new React Query hooks**. Tenant base table count: 206 → **217**. Catalogue stays at 450. IAM cache: Teacher 54 → 58, Parent 27 → 29, Student 28 → 29, Staff 67 → 70. This is the first cycle where parents are active schedulers rather than passive recipients: the parent-teacher conference (PTC) slot grid is parent self-service. The module bridges Cycle 10's IEP plans (via `mtg_iep_meeting_records.iep_plan_id` soft FK) and resolves Cycle 11's `svc_mtss_team_meetings.meeting_id` soft FK.
...
## REVIEW-CYCLE15 Round 1 fixes (2026-05-06)

Round 1 of REVIEW-CYCLE15-CHATGPT (against `cycle15-complete` at `93b1bd5`) returned **Reject pending fixes**. Three BLOCKING row-scope / sensitive-data issues plus five MAJOR follow-ups. The fix commit closes all 3 BLOCKING items + 4 of the 5 MAJORs in code; MAJORs 6 + 8 are recommendation-class and move to the Phase 2 punch list as items 20 + 21. Triage table + verification trail in `REVIEW-CYCLE15-CHATGPT.md`.

**(BLOCKING 1 — IEP record privacy)** `IepMeetingRecordService.assertHealthAccess()` allowed any caller with tenant-scoped `hlt-001:read` to read every IEP meeting record in the tenant. The seed grants Parent `hlt-001:read` for the child-health summary surface, so a parent could call `GET /meetings/<iepMeetingId>/iep-record` and receive `outcomes_summary` + `attendee_roles` + linked IEP plan metadata for any student. Fix: new private `canReadRecord(actor, meetingId, studentId)` with three tiers — admin all / counsellor (cou-001:write at the role level) own active caseload via `svc_caseloads` / non-counsellor STAFF + meeting participation + `hlt-001:read` for the health-team operator path. Everyone else gets a collapsed 404 don't-leak-existence. `create()` and `patch()` write authority restricted to admin or counsellor with the student on own active caseload; the create + patch services also reject the request when a counsellor tries to author an IEP record for a student outside their caseload. Parents and teachers no longer reach this surface.

**(BLOCKING 2 — slot identity leak)** `SlotService.listForMeeting(meetingId)` returned `bookedBy` + `bookedByName` + `bookedAt` for every slot to every reader, leaking other parents' booking identity to anyone with `mtg-002:read`. Fix: signature changed to `listForMeeting(meetingId, actor)`; the service resolves whether the caller is admin or the meeting organiser. Those two see the full identity columns. Other readers see `is_booked` plus their own booking with identity columns populated, while every other parent's booking has `bookedBy` + `bookedByName` + `bookedAt` + `notes` nulled in the response. Controller resolves the actor via `actors.resolveActor(...)` and passes it through.

**(BLOCKING 3 — agenda enumeration)** `GET /meetings/:id/agenda` was gated on `mtg-001:read` with no row scope. Any user with that permission could read agenda items for unrelated meetings (including IEP review and staff meetings) by guessing the meeting UUID. Fix: `AgendaService.listForMeeting(meetingId, actor)` calls `meetings.isParticipantOrOrganiser(meetingId, actor.accountId)` first; non-participant non-admin callers receive a collapsed 404 NotFoundException matching the row-scope-don't-leak-existence pattern from REVIEW-CYCLE13 BLOCKING 2 + REVIEW-CYCLE10 + REVIEW-CYCLE14. Controller resolves the actor and passes it through.

**(MAJOR 4 — tenant validation)** `participantIds` (on POST /meetings + POST /meetings/:id/participants), `presenterId` (on agenda create + patch), and `assigneeId` (on action-item create) were not validated against the calling tenant's user projections. A tenant A admin could land a meeting roster on a tenant B user by guessing UUIDs. Fix: shared `MeetingService.assertAccountInCurrentTenant(accountId, fieldName)` joins through `sis_students` / `sis_guardians` / `hr_employees` to confirm the supplied `platform_users.id` has a projection in the calling tenant. Mirrors Cycle 6.1 Profile + Cycle 7 Task patterns. Applied at every write site listed above; bogus UUIDs return 400 with the field name in the message ("participantId does not belong to a user in this school").

**(MAJOR 5 — slot booking authority)** `PATCH /meeting-slots/:id/book` required only `mtg-002:read`. Teachers and staff hold `mtg-002:read` and could use the parent self-service path. Fix: `SlotService.book` now refuses non-GUARDIAN actors unless they are school admin OR the meeting organiser (so a teacher running the conference can book on behalf of a parent who phoned in). Teachers + non-organiser staff are explicitly refused with the redirect message "Slot booking is parent self-service. Admins and the meeting organiser can also book on behalf."

**(MAJOR 7 — recording consent admin bypass)** `RecordingService.giveConsent()` allowed school admins to consent even if they were not on the meeting roster; the consent then did not count toward `totalParticipants` (because totals are based on `mtg_meeting_participants`), creating confusing rows. Fix: removed the `actor.isSchoolAdmin` bypass. Consent now requires the actor to be a meeting participant or the organiser. An admin who is not on the roster cannot consent through this surface — a future override path with audit can be added if real-school operations need it.

**(MAJOR 6 — DEFERRED)** Slot cancellation removes the parent's participant row when no other slot is booked, but cannot distinguish "auto-created by booking" from "manually added" rows. Recommendation-class. Pre-pilot fix: add `source TEXT NOT NULL DEFAULT 'MANUAL'` 2-value CHECK column on `mtg_meeting_participants`; SlotService.book inserts with `source='SLOT_BOOKING'`; cancel only removes participant rows where source matches. Documented as CLAUDE.md punch list item 20.

**(MAJOR 8 — DEFERRED)** CAT script reads as a reproducible script rather than a live verification record with captured outputs. Recommendation-class, not a code blocker. Documentation pattern improvement for future sensitive-cycle CATs. Documented as CLAUDE.md punch list item 21.

**Live verification on `tenant_demo` 2026-05-06.** All five code-level fixes verified end-to-end:

- BLOCKING 1 — Parent David `GET /meetings/<iepMeetingId>/iep-record` → 404 (was: full IEP record). Teacher Rivera (no caseload) → 404. Counsellor Hayes (Maya on own caseload) → full record. Admin → full record. Counsellor creating IEP record for non-caseload student → 403.
- BLOCKING 2 — Parent A sees own booking with name; other parents' bookings null'd. Organiser + admin see full identity columns.
- BLOCKING 3 — Teacher GET /meetings/<iepMeetingId>/agenda → 404 (was: 2 leaked agenda items). Hayes (organiser) → full agenda.
- MAJOR 4 — bogus UUID on participantId/presenterId/assigneeId → 400 with field name in message; real UUID → 201.
- MAJOR 5 — Teacher PATCH /meeting-slots/:id/book → 403 with redirect message; Parent → 201; Admin / organiser → 201.
- MAJOR 7 — Non-participant admin POST /meeting-recordings/:id/consent → 403; participant admin → 200.
