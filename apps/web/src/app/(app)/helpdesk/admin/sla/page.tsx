...
export default function SlaDashboardPage() {
  ...
  const stats = useMemo(
    () => (tickets.data ? computeDashboard(tickets.data) : null),
    [tickets.data],
  );
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
            <StatCard
              ...
              hint={
                stats.liveByPriority.CRITICAL + stats.liveByPriority.HIGH === 0
                  ? 'No urgent rows'
            ...
            {stats.breachedLive.length === 0 ? (
              ...
              <ul className="space-y-2">
                {stats.breachedLive.map((t) => {
                  ...
                  return (
                    <li key={t.id} className="rounded-md border border-rose-200 bg-rose-50/40 p-3">
                      <Link href={'/helpdesk/' + t.id} className="flex items-center gap-3 text-sm">
                  ...
                  {stats.byCategory.map((c) => (
                    <tr key={c.categoryId}>
                      ...
                      <td
                        className={cn(
                          'py-2 text-right',
                          c.breached > 0 ? 'text-rose-700' : 'text-gray-400',
                        )}
                      >
                        {c.breached}
                      </td>
