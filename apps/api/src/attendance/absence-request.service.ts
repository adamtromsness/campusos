    ...
    return this.tenantPrisma
      .executeInTenantTransaction(async (tx) => {
        ...
        await tx.$executeRawUnsafe(
          ...
          dto.reasonCategory,
          dto.reasonText,
          dto.supportingDocumentS3Key ?? null,
          initialStatus,
        );
    ...
    return this.tenantPrisma
      .executeInTenantTransaction(async (tx) => {
        ...
        if (existing.length === 0)
          throw new NotFoundException('Absence request ' + id + ' not found');
        if (existing[0]!.status !== 'PENDING') {
          throw new BadRequestException(
            'Request is already ' + existing[0]!.status + ' and cannot be re-reviewed',
          );
        }
