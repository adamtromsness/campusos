...
export class EngagementScoreWorker implements OnModuleInit, OnModuleDestroy {
  ...
  async computeComponents(
    ...
    return {
      ...
      communication: await this.communicationComponent(client, accountHolderId),
      conference: await this.conferenceComponent(client, schoolId, accountHolderId),
      volunteer: await this.volunteerComponent(client, accountHolderId),
  ...
  // communication_component — msg_message_reads / msg_messages addressed to parent
    ...
    try {
      const rows = (await client.$queryRawUnsafe(
        `SELECT
           ...
           COUNT(DISTINCT mp.message_id)::int AS total_count
         FROM msg_thread_participants tp
         JOIN msg_messages mp ON mp.thread_id = tp.thread_id
         LEFT JOIN msg_message_reads r
           ON r.message_id = mp.id AND r.user_id = tp.user_id
  ...
  // normalised against a ceiling of 10 events.
  ...
  // or PARTIALLY_PAID and not OVERDUE.
