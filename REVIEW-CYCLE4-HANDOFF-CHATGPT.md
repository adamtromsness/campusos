...
11. **The `LeaveService.list` non-admin filter is silent (200, scoped) not 403.** A teacher passing `?employeeId=other` gets their own rows, not a 403, because the row-scope is the right contract for a benign read attempt. The actual approval *write* path is the gated one.
