...
| ---------- | ----------- | --------------------------------- | ------------------------------------------------------------------------------ |
| BLOCKING 1 | reliability | Kafka consumer swallows errors    | Fixed — `KafkaConsumerService` rethrows + bounded retry + DLQ                  |
| BLOCKING 2 | reliability | Delivery worker SENT-as-in-flight | Fixed — `NotificationDeliveryWorker` uses `PROCESSING` state + stale-row sweep |
| MAJOR      | security    | `isManager()` overly broad        | Fixed — gate on `com-002:write` instead of `personType === 'STAFF'`            |

### BLOCKING 1 — Kafka consumer error semantics

**Before:** `apps/api/src/kafka/kafka-consumer.service.ts::eachMessage` caught every handler error, logged it, and returned. KafkaJS then advanced the consumer offset even though the handler failed. The notification consumers' "claim-after-success" idempotency was correct, but the shared wrapper's swallow meant a transient DB blip during recipient resolution would silently drop the event without redelivery.

**After:** the wrapper rethrows handler failures so KafkaJS retains the offset and re-delivers. To prevent a poison message from blocking the partition forever, an in-memory `Map<groupId:topic:partition:offset, attempts>` tracks per-message attempts. After `MAX_HANDLER_ATTEMPTS` (default 5, configurable via `KAFKA_MAX_HANDLER_ATTEMPTS`) the message is parked into `platform.platform_dlq_messages` (with `event_id`, `tenant_id`, original headers + payload, `error_class`, `error_message`, `retry_count`) and the throw is swallowed so the offset advances. Malformed JSON is parked directly without retry — it can't succeed on a redeliver. `KafkaConsumerService` now depends on `TenantPrismaService` to write DLQ rows.

### BLOCKING 2 — Delivery worker in-flight state

**Before:** `NotificationDeliveryWorker.processPendingForTenant` did a single transaction that SELECTed PENDING rows FOR UPDATE SKIP LOCKED and immediately UPDATEd them to `SENT` (with `attempts++`). The actual delivery (Redis ZADD + `msg_notification_log` write) ran AFTER the tx committed. If the process crashed in the gap, the row was permanently SENT with no log row and no retry path — silent loss.

**After:** the worker uses a proper `PROCESSING` in-flight state via the new tenant migration `010_msg_queue_processing_state.sql`. Three phases per tick:

- **Phase 0 — stale-row sweep.** UPDATE rows where `status='PROCESSING' AND processing_started_at < now() - INTERVAL '5 minutes'` back to `PENDING` (clearing `processing_started_at`, fresh `scheduled_for`). This recovers rows whose worker died mid-flight. Attempts are preserved so MAX_ATTEMPTS still bounds total retries — including ones lost to crashes.
- **Phase 1 — atomic claim.** Inside one tx: SELECT FOR UPDATE SKIP LOCKED LIMIT 25 + UPDATE status='PROCESSING' + processing_started_at=now() + attempts++. The lock is released when the tx commits.
- **Phase 2 — deliver outside the lock.** Per-row: `deliver()` writes the Redis sorted set + `msg_notification_log` row. On success → UPDATE status='SENT', sent_at=now(), processing_started_at=NULL. On failure → existing `markFailure` path flips back to PENDING with backoff (or FAILED at MAX_ATTEMPTS exhaust), also clearing processing_started_at.

The migration extends the queue's status CHECK constraint to include `PROCESSING`, adds a `processing_started_at TIMESTAMPTZ` column, and adds a partial index `WHERE status='PROCESSING'` to keep the sweep cheap. Tenant base table count: unchanged at 57 (constraint + column only).

**Migration gotcha (documented for the next cycle):** the SQL splitter in `provision-tenant.ts` filters statements that start with `--` after trim. A migration file beginning with a multi-line `--` header silently drops its first statement. The new migration uses `/* */` block comments to avoid the trap. CLAUDE.md flags this gotcha; it tripped the first attempt at applying 010.

`STALE_PROCESSING_SECONDS` is configurable; current default 300s is generous for IN_APP delivery (Redis ZADD is sub-millisecond) and bounded enough that a real crash recovers within one minute (next tick) plus the 5-minute window — net <6 minutes for a worst-case retry.

### MAJOR — Announcement manager scope

**Before:** `AnnouncementService.isManager(actor)` returned `actor.isSchoolAdmin || actor.personType === 'STAFF'`. The IAM seed gives Staff just `com-002:read`, so a staff member would have been treated as a manager and could read every tenant-wide announcement (and via `?includeDrafts=true`, every draft including ones authored by other people) despite holding only the read permission.

**After:** `isManager` is now async and calls `permissionCheckService.hasAnyPermissionInTenant(actor.accountId, tenant.schoolId, ['com-002:write'])` after the school-admin shortcut. The check uses the same scope chain as every other tenant-bounded admin assertion (school + platform per ADR-036). All four call sites in `announcement.service.ts` (`list`, `getById`, `create`, `update`) await the new helper. Stats authorization (`getStats`) was already author-or-admin (`isAuthor || actor.isSchoolAdmin`) and didn't need changing. Dependency: `AnnouncementService` now injects `PermissionCheckService` from `IamModule` (already imported by `AnnouncementsModule`, already exported by `IamModule`, no module wiring change needed).

### CI / lint fixes shipped alongside

`pnpm format:check` was failing on 30 files (`prettier --check`); `pnpm format` (Prettier `--write`) reformatted them. The lint+typecheck CI job and the unit-test CI job now both pass locally — `pnpm --filter @campusos/api build` ✔, `pnpm --filter @campusos/web build` ✔, `pnpm test` ✔ (7 tests across 2 files). The `Lint & Type Check` job was failing on the prior push because of the prettier mismatch; should now go green.

### Re-verified CAT scenarios

After the fixes, the full Cycle 3 CAT harness was re-run against `tenant_demo`. All 7 scenarios still pass:

- Tardy → bell badge (`unreadCount=15` for parent, `attendance.tardy student=Maya Chen status=TARDY` at the top)
- Grade publish → fan-out (`grade.published rows = 3` for parent, `4` for student)
- Direct messaging compose + reply + mark-read
- BLOCK keyword `shit` → 422 + `BLOCKED / URGENT / {shit}` log row
- ALL_SCHOOL announcement → 5 recipients fanned out → mark-read flips `readCount=1 readPercentage=20`
- `is_enabled=false` pref → BEFORE=4 AFTER=4 (no new queue row)
- 3 permission denials (403 / 403 / 404)

Queue state inspection post-run: 80 rows all `SENT`, zero stuck in `PROCESSING` — confirms the new state machine drains cleanly.
