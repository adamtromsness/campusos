...
**Test coverage in progress — Wave 1 (financial) underway.** Tier 0 (integration test harness at `apps/api/test/integration/`) is built and operational. Tiers 1–7 (per-module unit + integration coverage targets per `docs/architecture/campusos-test-coverage-plan.html`) are the active engineering work, executing through the wave-by-wave plan in `docs/campusos-test-strategy-v3.html`. 2632 unit tests pass across 117 spec files (54 pre-existing skips, 0 failures), plus 435 DB-backed integration tests passing (5 documented `it.skip` flagging real service bugs — see `docs/reviews/handoffs/HANDOFF-WAVE1.md`). **Wave 1** (m83-finance, m84-payments, m86-procurement): m83-finance complete (chart-of-accounts + gl-posting IMMUTABLE + budget-management atomic transfer + gl-reconciliation 7 check types); m84-payments invoice-lifecycle + payment-processing + refunds-reversals (IMMUTABLE pay_credit_notes + pay_payment_reversals + outbox for refund/credit-note/reversal events) landed; payment-plans / lunch-accounts / financial-aid / late-fees / m86-procurement cross-school + IMMUTABLE additions still pending.

**Build state:**

- `pnpm --filter @campusos/api build` — 0 errors (`nest build`)
- `pnpm --filter @campusos/api test` — 2632 / 2632 passing (+ 54 skipped)
- `pnpm --filter @campusos/api test:integration` — 435 / 435 passing (+ 5 documented skips)
