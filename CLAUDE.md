...
**Test coverage in progress — Wave 1 (financial) underway.** Tier 0 (integration test harness at `apps/api/test/integration/`) is built and operational. Tiers 1–7 (per-module unit + integration coverage targets per `docs/architecture/campusos-test-coverage-plan.html`) are the active engineering work, executing through the wave-by-wave plan in `docs/campusos-test-strategy-v3.html`. 2703 unit tests pass across 119 spec files (54 pre-existing skips, 0 failures), plus 395 DB-backed integration tests passing (2 documented `it.skip` flagging real service bugs — see `docs/reviews/handoffs/HANDOFF-WAVE1.md`). **Wave 1** (m83-finance, m84-payments, m86-procurement): m83-finance chart-of-accounts + gl-posting (IMMUTABLE trigger on fin_gl_entries) + budget-management (atomic transfer + outbox-in-tx) + gl-reconciliation (7 check types) landed; m84-payments invoice-lifecycle (outbox-in-tx for pay.invoice.created + pay.debt.written_off) + payment-processing (outbox-in-tx for pay.payment.received + Stripe stub + auth contract) landed; refunds-reversals / payment-plans / lunch-accounts / financial-aid / late-fees / m86-procurement cross-school + IMMUTABLE additions still pending.

**Build state:**

- `pnpm --filter @campusos/api build` — 0 errors (`nest build`)
- `pnpm --filter @campusos/api test` — 2703 / 2703 passing (+ 54 skipped)
- `pnpm --filter @campusos/api test:integration` — 395 / 395 passing (+ 2 documented skips)
