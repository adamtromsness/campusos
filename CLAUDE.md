...
apps/api/src/classroom/  → M21 Classroom (Cycle 2): assignment + category CRUD (Step 4) + submissions, grading (single/batch/publish), gradebook reads, and progress notes (Step 5). 4 services + 6 controllers; emits cls.submission.submitted, cls.grade.published, cls.grade.unpublished, cls.progress_note.published. Step 6 wires the matching gradebook snapshot Kafka consumer.
