...
**Cycle 2 (Classroom + Assignments + Grading) is IN PROGRESS — Step 1 done.** `005_cls_lessons_and_assignments.sql` adds the first 7 M21 Classroom tables (`cls_lesson_types`, `cls_lessons`, `cls_assignment_types`, `cls_assignment_categories`, `cls_assignments`, `cls_assignment_questions`, `cls_answer_key_entries`) to the tenant schema. Idempotent migration; 10 intra-tenant FKs; zero cross-schema FKs. Steps 2–10 (submissions/grading schema, seed, NestJS module, Kafka snapshot worker, teacher/student/parent UI, vertical-slice CAT) remain.
...
apps/api/src/classroom/  → M21 Classroom (Cycle 2): assignments, submissions, grading, gradebook snapshots — schema lands in Steps 1–2; module lands in Steps 4–5
...
- **Tenant schema** (30 base tables after Cycle 2 Step 1): 5 from Cycle 0 foundation (school_config, school_feature_flags, grading_scales, custom_field_definitions, custom_field_values) + 18 SIS tables from Cycle 1 (sis_academic_years, sis_terms, sis_departments, sis_courses, sis_classes, sis_class_teachers, sis_enrollments, sis_families, sis_students, sis_staff, sis_guardians, sis_student_guardians, sis_family_members, sis_emergency_contacts, sis_student_notes, sis_absence_requests, sis_attendance_records, sis_attendance_evidence) + 7 Classroom tables from Cycle 2 Step 1 (cls_lesson_types, cls_lessons, cls_assignment_types, cls_assignment_categories, cls_assignments, cls_assignment_questions, cls_answer_key_entries). Plus 36 partition objects under sis_attendance_records (4 year partitions × 8 hash leaves + 4 year parents). Cycle 2 Step 2 adds 8 more Classroom tables (submissions/grading) for a planned final tenant total of 38 base tables.
