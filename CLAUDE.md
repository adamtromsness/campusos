...
- **Long edit forms use a bottom-anchored save bar.** Tabbed / scrollable profile forms commit through the viewport-fixed `apps/web/src/components/ui/StickySaveBar.tsx` (Discard + Save Changes, rendered only while the form is dirty) rather than an inline mid-page button, so the save action is always reachable. Used by the parent `/profile` tabs, the child profile `/family/children/[id]` Account + Contact tabs, and the `/family/settings` Family / Addresses / Health tabs. Each form keeps its OWN save scope — one `StickySaveBar` per dirty form, only one visible at a time (you can only edit one tab at a time). Per-row immediate-persist controls (phone/email/emergency-contact lists, Family Structure relationship edits) and separate-tab saves (child Medical/Dietary) keep their own actions and are never folded into a form's sticky bar. Keep a hidden `type="submit"` button so Enter-to-save still works.
...
- **Personas (2026-05-23):** A person's runtime role (PARENT / STUDENT / STAFF / SUBSTITUTE / ALUMNI / COMMUNITY) is DERIVED from projection tables — never assigned by an admin. `PersonaResolutionService` (in m00-platform/iam) reads `platform_family_children` (LINKED), `hr_employees` (ACTIVE / ON_LEAVE), `sis_students` (ENROLLED) via `platform_students`, `platform_substitute_profiles`, `alm_alumni_profiles`, and `grp_members`, then caches the result in `platform_personas`. `/auth/me` returns `{ user, activePersona, personas, permissions }`; the legacy `personType` field is REMOVED from the API response. Frontend reads `user.activePersona?.type` and uses the new persona vocabulary (note: GUARDIAN → PARENT). Internal `actor.personType` (read from `iam_person.person_type`) is unchanged.
...
- **Platform schema** (~33 tables): organisations, schools, iam_person, platform_users, platform_students, platform_families, platform_family_children, platform_person_relationships, platform_guardian_edit_consent, platform_personas, platform_invitations, platform_person_phones, platform_person_emails, roles, permissions, iam_scope, iam_role_assignment, iam_effective_access_cache, platform_push_tokens, platform_dlq_messages, platform_outbox, platform_tenant_routing, platform_event_consumer_idempotency, platform_audit_log, platform_reference_health, signature/breach/SAR governance tables. Managed by Prisma at `packages/database/prisma/platform/schema.prisma`. `platform_family_children` (PLACEHOLDER → PENDING_LINK → LINKED), `platform_personas` (cached persona set per person), and `platform_invitations` (employee / child-link / parent-link tokens) back the persona-registration feature added 2026-05-23. `platform_person_phones` + `platform_person_emails` are the per-person multi-row contact lists — each carries a type, an isPrimary flag (one-per-person, partial UNIQUE index), and the family-children read path COALESCEs the primary email in so `FamilyMemberDto.email` always reflects the canonical contact email without joining at every caller. Login email (`platform_users.email`) stays separate and is not edited from `/profile`. Both tables are keyed by `iam_person.id` and serve adults (via `/profile/me/{phones,emails}`) and LINKED children (via `/family/children/:id/{phones,emails}`) from the same surface. `platform_person_relationships` (added 2026-05-30) is the **family-structure** graph — biological / legal / step-family relationships between people, **distinct from the household model** (households = who lives together + manages accounts; family structure = who is biologically/legally related). Stored bidirectionally (`RelationshipService` in m00-platform/iam auto-creates the reciprocal: parent↔child, guardian↔ward, symmetric spouse); siblings (FULL / HALF / STEP / ADOPTIVE) are **derived at read time from shared parents, never stored**. `related_person_id` is nullable with a `related_person_name` fallback for non-CampusOS people (no placeholder iam_person, no reciprocal). TEXT+CHECK enums for `relationship_type` (15) + `custody_arrangement`; partial UNIQUE indexes for linked vs name-only dedup. Endpoints live at `/people/:personId/{relationships,family-tree}` (+ `:id/verify`, school-admin only); auth is row-level, not a static permission code. **View** (GET) = self / active guardian / school admin. **Edit** (POST/PATCH/DELETE) = `canEditFamilyStructure` in `relationship.auth.ts` — **parent/guardian-only**, keyed off the caller's **derived personas** (not `iam_person.person_type` — a self-registered parent's person_type need not be `GUARDIAN`): edit-eligible if ANY active persona is parent/guardian-type (`PARENT`, the only such persona the model defines) AND they are the profile owner or an active guardian of the person (`RelationshipService.isActiveGuardianOf` = guardian-role household membership ∪ a current parent/guardian relationship). The controller fetches personas via `PersonaResolutionService.getActivePersonas`. Students (even adult/self), staff/admins, and Substitute-only accounts are never editors; an admin's only mutation is verify. The household path requires a guardian-role member (`PARENT`/`GUARDIAN`/`HEAD_OF_HOUSEHOLD`/`SPOUSE`) so a co-resident student can't edit; this is also the bootstrap — the parent who creates a child account is the family's `HEAD_OF_HOUSEHOLD`, so they can edit that child's structure immediately. GET responses carry a server-computed `canEdit` flag the UI uses for rendering (never the security boundary). Web: a Family Structure section on the child + adult profile Account tabs (edit affordances gated on `canEdit`), and a read-only blended **family-tree diagram** (`FamilyTreeView`) shared by `/family/[personId]/structure` and `/family/tree`. **Guardian editing — age + consent model (2026-06-02):** who may edit a child/dependent's account is now AGE-DRIVEN, with a post-18 consent layer — the "Independent" flag is **descriptive only** (a self-login indicator) and no longer gates anything. ONE predicate, `RelationshipService.canGuardianEdit(guardianPersonId, subjectPersonId)`, governs every section (identity / medical / dietary / emergency / family structure): it returns false unless the caller is an active guardian (`isActiveGuardianOf` = household ∪ graph, unchanged), then — **under 18 → true unconditionally** (ignores Independent; unknown DOB treated as a minor), **18+ → true only while consent is not REVOKED**. `canEditFamilyStructure` now delegates its non-self branch to `canGuardianEdit` (keeping the PARENT-persona + self gate); the family-children write paths (`update` identity + `assertCanEditLinkedChild` for medical/dietary/emergency/phones/emails) all funnel through it. The old INDEPENDENT-403 throw on child identity edits is gone; the parallel `updateMember` INDEPENDENT throw stays (it protects adult co-guardian *members*, not dependents — out of scope). `FamilyChildDto.canEdit` is the server-computed, caller-relative authority the web gates ALL edit affordances on (decorated via `decorateCanEdit`); `accessLevel` is kept for descriptive badges/labels only. **Consent state** lives in `platform_guardian_edit_consent` (guardian_person_id, subject_person_id, state GRANTED|REVOKED, UNIQUE pair; migration 20260602000000). An ABSENT row reads as GRANTED — the carryover default, so no data backfill is needed and no one is locked out on deploy. Rows are materialised only for (a) a NEW guardian link formed while the subject is already 18+ → REVOKED (`addRelationship`, so a new guardian doesn't get silent access to an adult), or (b) an explicit grant/revoke by the adult. **The adult's control:** `GET /people/:personId/guardian-access` + `PATCH /people/:personId/guardian-access/:guardianId { state }` — **subject-only and 18+** (403 otherwise; a guardian can never set their own access to an adult). Web: `GuardianAccessSection` ("People who can edit my account") on the 18+ person's own `/profile` Account tab, with Revoke/Grant per guardian; under-18s never see it. **Family-tree diagram (2026-06-01):** `GET /people/:id/family-tree` returns a single-generation GRAPH (not the old bucketed lists) — `{ rootPersonId, canEdit, parents[], children[] }` where `parents` is the deduped union of every distinct parent and each child carries its own `parentLinks` (so a child can be biological to one parent and step to another; type/custody live on the link). A child's known-but-unspecified second parent is a `parentLink` with `parentPersonId`+`parentName` null (the meaningful empty slot — never omitted; padded to 2). A childless root renders as its own child (subject + their parents) so student profiles still show a tree. `FamilyTreeView` renders an SVG box-and-line diagram (parent row → child row, dashed placeholder slots); grandparents/siblings/spouses are out of scope for this view (still listed in `GET /relationships`). Read-only regardless of `canEdit`. **Union-junction layout (2026-06-01):** the diagram no longer draws one edge per parentLink (which crisscrossed for intact families). Children are grouped by the unordered SET of their real parents; each group renders ONE union — the group's parents joined by a short horizontal line, a single drop to a horizontal sibling bar, one short drop per child — so the common case (all kids share two parents) has ZERO crossings. Blended families produce multiple unions; a parent shared across groups appears once (barycenter-ordered) and is the only place a residual crossing can occur. A single-parent group drops from the real parent with no phantom line to the empty slot; a childless root (no real parents) renders boxes only. The canvas is sized to the wider of the two rows (no clipping) and switches to a horizontally scrollable container past 4 children rather than shrinking text; child boxes show `FirstName · age` + a one-word relationship subtitle (no custody/full-type strings → no truncation), parents show name + role. No graph-layout lib — the grouping does the work. **Clickable person nodes (2026-06-01):** each node carries a server-resolved, access-gated `profileUrl` (on `FamilyTreeParentDto` + `FamilyTreeChildDto`); a node is clickable (router-navigates to that profile, keyboard-focusable, Enter/Space, `aria-label "View {name}'s profile"`, hover/focus stroke) iff `profileUrl` is non-null — the client never re-checks permissions or resolves routes. The controller decorates the tree via `RelationshipService.resolveProfileUrls(personIds, viewerPersonId, isSchoolAdmin)`: viewer's own node → `/profile`; school admin → `/profile/:personId` (holds `usr-001:admin`); a guardian's LINKED managed child → `/family/children/:familyChildId` (resolved from `platform_family_children` — note this is the family-child record id, **not** the iam_person id); otherwise null. Name-only parents, placeholder slots (no node), and real people the viewer can't route to stay inert — avoiding dead-end 403s. Navigation only; the tree stays read-only. **Set Father/Mother modal self-selection (2026-05-30):** the modal's people search calls `GET /people/search?includeSelf=true` (default still excludes the caller, so the emergency-contact flow is unchanged) and offers a "This is me — set myself as {father/mother}" shortcut, so a parent can set themselves — the primary use case. On save, `RelationshipService.addRelationship` is **upgrade-in-place**: when the selected CampusOS person already has an active edge to the subject (e.g. the bootstrap household guardian who also holds a `LEGAL_GUARDIAN` graph row), it UPDATEs that row (and its reciprocal) to the chosen parentage type with `is_legal_custody` forced on, rather than inserting a duplicate or 409ing — the parent appears once ("Biological father" with custody), not twice. Because `PARENT_TYPES` (the `isActiveGuardianOf` graph signal) spans both `LEGAL_GUARDIAN` and every parentage type, the upgrade never strips the editor's own edit rights. **Account creation — DOB/gender + age variant (2026-05-30):** provisioning a real account via `createAccountForChild` / `createAccountForMember` (m00-platform/households) now REQUIRES date of birth + gender (400 with a field-scoped message; future DOB rejected). Enforced only at account creation, not on the PLACEHOLDER `create()` stub, seeds, or other-domain account paths (auth.register / SIS / HR / config import) which own their own validators. Age sets `iam_person.person_type`: ≤18 → STUDENT, >18 → GUARDIAN (the member path's old EXTERNAL is gone). `person_type` is a Postgres ENUM, so bound params in raw SQL are cast `::"PersonType"`. Personas stay DERIVED, never assigned — the >18 "also a student" choice is a UI-variant/redirect concern with no durable backend state. The web `/family/add-child` page is generalised to an add-person form (profile-style layout, inclusive required gender select, age-based redirect to `/family/children/:id`). **Privacy-safe duplicate detection:** `POST /people/check-duplicate` (m00-platform/iam, auth-only, Redis rate-limited) reports a STRONG match only — exact email OR normalized first+last AND exact DOB; partial-name never matches (enumeration guard). It returns a MINIMAL descriptor ({exists, displayName 'Given L.', context coarse-role, alreadyManagedByCurrentUser}) — never email/DOB/contact. The add-person form offers a direct link only when the match is already managed by the caller; cross-owner claim-approval is deferred. **Your-Family completion + primary guardian (2026-06-01):** the family-profile completion (% / per-section "N items remaining" / yellow checklist) is a single CLIENT-side source — `useCompletionState` in `apps/web/.../family/settings/page.tsx`; there is no backend completeness service. "Family name set" and "Communication preferences customised" were dropped from the criteria (display name is optional; comm prefs seed to operational defaults — all 8 `platform_family_contact_preferences` categories route to the creator-primary on first read, none is a consent/marketing channel). The guardian-profile criterion reads `FamilyMemberDto.primaryPhone`, which now COALESCEs the `platform_person_phones` multi-row list before the `iam_person.primary_phone` cache (symmetric with email) — fixing a false negative where an ACTIVE guardian's number lived only in the list. **Change primary guardian:** `PATCH /api/v1/families/:familyId/primary-guardian { guardianPersonId }` (`FamiliesController` → `FamilyChildrenService.setPrimaryGuardian`) atomically demotes the old + promotes the new (the `(family_id) WHERE is_primary_contact = true` partial UNIQUE keeps exactly one); target must be an ACTIVE guardian of the family (else 400), caller must be a member (else 404). "Primary" is a contact label only — it touches just `is_primary_contact` and is decoupled from `canEditFamilyStructure`/`isActiveGuardianOf`. Web: a "Make primary" action on each non-primary ACTIVE guardian row (any active guardian may reassign). **Gender / mailing / family-medical fixes (2026-06-01):** (1) Gender is a fixed three-option set — `MALE | FEMALE | NOT_SPECIFIED` — shared via `@campusos/shared` (`GENDERS`, `GENDER_LABELS`, `normalizeGender`); the API normalises on read AND write (legacy `'F'`→FEMALE, `'M'`→MALE, anything else→NOT_SPECIFIED) and a backfill (20260601010000) canonicalises `iam_person.gender` + `platform_family_children.gender` (the admin per-tenant `sis_student_demographics.gender` is untouched). `iam_person.gender` is plain TEXT (not the `PersonType` ENUM — that gotcha is the person_type column, above). (2) Mailing address has a `mailing_address_source` (FAMILY|CUSTOM) on `platform_family_children` + `iam_person` (20260601020000) giving it the same Use-family/Use-custom toggle as home; FAMILY inherits the family mailing address (falling back to family home when the family keeps mailing == home), CUSTOM keeps the same-as-physical boolean nested under it. (3) BOTH the child Medical ("Use custom") and the adult/parent profile Medical ("Use my own") "Use family" views inherit the family's explicit `hasFamilyDoctor`/`hasInsurance` flags (carried on `ChildMedicalInfoDto` + `AdultMedicalInfoDto`, FAMILY mode only — null in CUSTOM) so a definitive "No family doctor/insurance on file" renders instead of blank dashes. The two are separate components + separate API paths (child: `getChildMedical`/`loadFamilyDoctorInsurance`; adult: `getMyMedical`/`loadFamilyDoctorInsuranceForPerson`) — the fix is applied to both.
